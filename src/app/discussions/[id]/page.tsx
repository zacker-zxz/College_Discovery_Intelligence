...
export default function DiscussionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!discussion) {
    return (
      <div className="app-frame py-16 text-center">
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Discussion Thread Not Found</h2>
        <Link href="/discussions" className="text-xs text-blue-600 dark:text-blue-400 underline mt-2 block">
  ...
  return (
    <div className="app-frame py-8 max-w-4xl mx-auto space-y-8">
      {/* Back Button */}
      <Link href="/discussions" className="inline-flex items-center gap-1 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors">
        <ArrowLeft className="w-4 h-4" /> Back to All Discussions
      </Link>

      {/* Main Question Card */}
      <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 p-6 md:p-8 space-y-5">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 dark:text-slate-500">Question ID: #{discussion.id.slice(0, 8)}</span>
          </div>
        ...
        <div className="text-xs md:text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-line bg-slate-50 dark:bg-slate-800/60 p-4 rounded border border-slate-100 dark:border-slate-800">
          {discussion.body}
        </div>
        ...
        {discussion.answers && discussion.answers.length > 0 ? (
          <div className="space-y-4">
            ...
            {discussion.answers.map((ans: any) => (
              <Card key={ans.id} className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 p-5 space-y-3">
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-900 dark:text-slate-100 text-xs">{ans.user?.name || "Verified Student"}</span>
                    {ans.user?.role === "ADMIN" && (
                  ...
                  <span className="text-[11px] text-slate-400 dark:text-slate-400">{new Date(ans.createdAt).toLocaleDateString()}</span>
                </div>
                <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-line">{ans.body}</p>
              </Card>
            ))}
          </div>
        ) : (
      ...
      {/* Submit Answer Form */}
      <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 p-6 space-y-4">
        <h4 className="font-bold text-slate-900 dark:text-slate-100 text-sm">Submit Your Answer</h4>

        {error && (
          <div className="bg-red-50 dark:bg-red-950/60 text-red-700 dark:text-red-300 text-xs p-3 rounded border border-red-200 dark:border-red-800">
            {error}
          </div>
        )}
