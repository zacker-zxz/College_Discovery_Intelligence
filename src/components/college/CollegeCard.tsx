...
export const CollegeCard: React.FC<CollegeCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="glass-card rounded-2xl p-5 shadow-card hover:shadow-glow transition-all duration-300 flex flex-col justify-between h-full relative overflow-hidden group border border-slate-200/80"
    >
      ...
      <div>
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-all border ${
              saved
                ...
                : "bg-white/80 text-slate-400 border-slate-200 hover:text-slate-700 hover:bg-slate-100"
            }`}
        ...
        <div className="space-y-1 mb-3">
          <Link
            ...
            className="font-extrabold text-slate-900 text-base leading-snug group-hover:text-blue-600 transition-colors block"
          >
            {college.name}
          </Link>
        ...
        {/* Key Metrics Grid */}
        <div className="grid grid-cols-3 gap-2 bg-slate-100/60 p-2.5 rounded-xl border border-slate-200/60 mb-4 text-center">
          <div>
            <span className="text-[10px] text-slate-500 block font-semibold uppercase tracking-tight">
              Avg Package
            </span>
            <span className="text-xs font-black text-slate-900 text-blue-700">{formattedAvgPkg}</span>
          </div>
          <div>
            <span className="text-[10px] text-slate-500 block font-semibold uppercase tracking-tight">
              Tuition Fee
            </span>
            <span className="text-xs font-black text-slate-900">{formattedMinFee}</span>
          </div>
          <div>
            <span className="text-[10px] text-slate-500 block font-semibold uppercase tracking-tight">
              Rating
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {college.courses.slice(0, 3).map((item: any) => (
                <span
                  key={item.id}
                  className="text-[11px] bg-white text-slate-700 font-semibold px-2 py-0.5 rounded-md border border-slate-200 shadow-2xs"
                >
      ...
      {/* Card Actions */}
      <div className="pt-3 border-t border-slate-200/80 flex items-center justify-between gap-2 mt-2">
        <Button
