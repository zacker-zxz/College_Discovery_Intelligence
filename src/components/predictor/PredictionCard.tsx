...
export const PredictionCard: React.FC<PredictionCardProps> = ({ prediction }) => {
  ...
  return (
    <Card hoverable className="border-slate-200 flex flex-col justify-between">
      <div>
        ...
        <div className="mb-2">
          <Link
            ...
            className="font-bold text-slate-900 text-base leading-tight hover:text-blue-600 transition-colors block"
          >
            {college.name}
          </Link>
          <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
            ...
            <span>•</span>
            <span className="font-medium text-slate-700">{college.institutionType}</span>
          </div>
        </div>

        {/* Course Info */}
        {course && (
          <div className="bg-slate-50 p-2.5 rounded border border-slate-100 mb-3 text-xs">
            <span className="text-[11px] text-slate-400 block uppercase font-medium">Program</span>
        ...
        <p className="text-xs text-slate-600 bg-slate-50/80 p-3 rounded border border-slate-100 leading-relaxed mb-4">
          {rationale}
        </p>

        {/* Cutoff Historic Range */}
        <div className="grid grid-cols-2 gap-2 text-xs border-t border-slate-100 pt-3 mb-2">
          <div>
            <span className="text-[10px] text-slate-400 block uppercase">Historical Range</span>
            <span className="font-mono font-semibold text-slate-800">
              {cutoffInfo.openingRank.toLocaleString()} – {cutoffInfo.closingRank.toLocaleString()}
            </span>
          </div>
          <div>
            <span className="text-[10px] text-slate-400 block uppercase">Avg Placement</span>
      ...
      <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
        <span className="text-[11px] text-slate-400">Quota: {cutoffInfo.quota}</span>
        <Link
          href={`/colleges/${college.slug}`}
