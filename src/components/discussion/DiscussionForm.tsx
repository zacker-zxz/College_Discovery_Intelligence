...
export const DiscussionForm: React.FC<DiscussionFormProps> = ({ onSubmit, colleges }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6 shadow-subtle space-y-4">
      <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
        <MessageSquare className="w-5 h-5 text-blue-600 dark:text-blue-400" />
        <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base">Ask the Campus Community</h3>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full px-3 py-2 text-xs border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-400 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 leading-relaxed"
          required
        />
      </div>
